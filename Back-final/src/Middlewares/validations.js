...
const validations = () => {
  return [
    ...
    body('telephone', 'Telephone is required').notEmpty().isString().trim().custom(validationStringContainNumbers),
    body('direction', 'Direction  is required').notEmpty().isString().trim(),
    body('dni', 'Dni is required').notEmpty().isString().trim().custom(validationStringContainNumbers),
